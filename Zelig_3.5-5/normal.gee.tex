...
\begin{Schunk}
\begin{Sinput}
RRR> z.out <- zelig(unem ~ gdp + capmob + trade, model = "normal.gee", id = "country", data = macro, robust=TRUE, corstr="AR-M", Mv=1)
RRR> summary(z.out)
\end{Sinput}
\end{Schunk}
...
\begin{Schunk}
\begin{Sinput}
RRR> x.high <- setx(z.out, trade = quantile(macro$trade, 0.8))
RRR> x.low <- setx(z.out, trade = quantile(macro$trade, 0.2))
\end{Sinput}
\end{Schunk}
