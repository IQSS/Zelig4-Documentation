...
\begin{Schunk}
\begin{Sinput}
RRR> coalition$cluster <- c(rep(c(1:62),5),rep(c(63),4))
RRR> sorted.coalition <- coalition[order(coalition$cluster),]
\end{Sinput}
\end{Schunk}
Estimating model and presenting summary:
\begin{Schunk}
\begin{Sinput}
RRR> z.out <- zelig(duration ~ fract + numst2, model = "gamma.gee", id = "cluster", data = sorted.coalition, robust=TRUE, corstr="exchangeable")
RRR> summary(z.out)
\end{Sinput}
\end{Schunk}
